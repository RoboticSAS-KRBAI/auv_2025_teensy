...
// // Covariance matrix
// float P_kf[2][2] = {{1,0},{0,1}};

// // Tuning
// float Q_yaw = 0.4f;    // Naikkan Q_yaw -> kurangi kepercayaan ke IMU
// float Q_bias = 0.01f;  // Naikkan Q_bias = bias/drift gyro cepat hilang, tapi bisa jadi sedikit noisy
// float R_compass = 0.1f; // Naikkan R -> kurang percaya ke compass, 
//                         //              Lebih percaya ke prediksi IMU (gyro)
...
// // // PRESET: IMU DOMINANT
// // // ==========================
// // float Q_yaw = 0.001f;     // kecil → percaya model gyro
// // float Q_bias = 0.0005f;   // bias dianggap stabil
// // float R_compass = 10.0f;  // besar → kurang percaya compass

// // // ==========================
// // // PRESET: COMPASS DOMINANT
// // // ==========================
// // float Q_yaw = 0.02f;     // lebih besar → model gyro dianggap kurang akurat
// // float Q_bias = 0.01f;    // bias cepat dikoreksi
// // float R_compass = 0.5f;  // kecil → percaya compass
...
// void kalmanYawUpdate(float yaw_meas)
// {
//     // update only
//     float y = calculate_heading_error(yaw_kalman, yaw_meas);
...
//     if (yaw_kalman < 0) yaw_kalman += 360;
//     if (yaw_kalman >= 360) yaw_kalman -= 360;
// }
...
//         gyro_z = sReg[GZ] / 32768.0f * 2000.0f;  // deg/s

//         s_cDataUpdate &= ~ACC_UPDATE;
//     }

//     // ===== KALMAN PREDICTION =====
//     yaw_kalman += (gyro_z - gyro_bias) * dt_loop;
...
//     P_kf[0][0] = P00 + dt_loop*(dt_loop*P11 - P01 - P10 + Q_yaw);
//     P_kf[0][1] = P01 - dt_loop*P11;
//     P_kf[1][0] = P10 - dt_loop*P11;
//     P_kf[1][1] = P11 + Q_bias*dt_loop;
...
//         float yaw_meas = raw / 10.0f;
//         yaw_meas = -yaw_meas;

//         if (yaw_meas < 0) yaw_meas += 360;
//         if (yaw_meas >= 360) yaw_meas -= 360;
