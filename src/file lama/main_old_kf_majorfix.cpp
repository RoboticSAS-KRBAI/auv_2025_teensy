...
// void kalmanInit() {
//   kf.x = 0.0f; kf.P = 1.0f;
//   kf.Q = 0.02f;  // Gyro trust
//   kf.R = 1.00f;  // Compass trust
// }
...
// // FIX #3: Hapus is_stable — dead zone dihapus total
...
// //   SEBELUM:
// //     - Tidak pakai dt → integral salah unit
// //     - Tidak ada anti-windup → integral bisa meledak
// //     - D-term bagi 10 (magic number) → tidak konsisten
// //     - Tidak ada low-pass filter D → spike setiap loop
// //   SESUDAH:
// //     - Pakai dt (0.05s = 20Hz sesuai timer)
// //     - Anti-windup ±100
// //     - D-term low-pass filter (0.7/0.3)
...
//       const float dt = 0.05f;  // 50ms = 20Hz (sesuai timer_timeout)
...
//       derivative = 0.7f * derivative + 0.3f * raw_d;
...
// // ====================================================================
// // FIX #2: PID instances GLOBAL — PERSISTENT!
// //   SEBELUM: PID pid_yaw(kp,ki,kd); dibuat ulang SETIAP loop
// //            → integral SELALU 0, last_error SELALU 0
// //            → I-term tidak bekerja, D-term spike
// //   SESUDAH: PID global, integral terus accumulate antar loop
// // ====================================================================
...
// //   SEBELUM: error_depth = is_stable_depth ? 0 : error_depth;
// //            → error 0.02m langsung di-nol-kan, PID berhenti!
// //   SESUDAH: Biarkan PID handle semua error secara natural

// // FIX #4: EMA filter sebagai pengganti rejection filter
// //   SEBELUM: if (delta_roll > 0.4) roll = last_roll;  → data DIBUANG
// //            → sensor stuck saat gerakan cepat
// //   SESUDAH: EMA smooth tanpa buang data
...
//   // Initialize RS485 for HWT3100 compass (Serial7)
...
//   // Initialize HWT905 (Serial6) - Pitch, Roll, dan Gyro Z
...
//   // ============================================================
//   // KALMAN FILTER STEP 1: PREDICT (Gyro Z dari HWT905)
//   // ============================================================
//   while (Serial6.available()) {
//     WitSerialDataIn(Serial6.read());
//   }
...
//     float kf_dt = (now - lastGyroTime) / 1000.0f;
//     lastGyroTime = now;

//     gyroZ = (float)sReg[GZ] / 32768.0f * 2000.0f;

//     if (kf_dt > 0.0001f && kf_dt < 0.2f) {
//       kalmanPredict(gyroZ, kf_dt);
//     }

//     s_cDataUpdate &= ~GYRO_UPDATE;
//   }

//   // FIX #4: Pitch & Roll dengan EMA filter (bukan rejection)
//   //   SEBELUM: pitch = sReg[AY] / 32768.0f * 16.0f; (langsung overwrite)
//   //            lalu: if (delta_pitch > 0.3) pitch = last_pitch; (buang data)
//   //   SESUDAH: EMA smooth — data baru selalu diperhitungkan
...
//   // ============================================================
//   // KALMAN FILTER STEP 2: UPDATE (Compass dari HWT3100 via Modbus)
//   // ============================================================
//   if (now - lastModbusTime >= MODBUS_RATE) {
//     lastModbusTime = now;

//     uint8_t result = nodemod.readHoldingRegisters(0x00DB, 4);

//     if (result == nodemod.ku8MBSuccess) {
//       int16_t rawYaw = nodemod.getResponseBuffer(3);
...
//   // ============================================================
//   // GUNAKAN HASIL KALMAN FILTER SEBAGAI YAW
//   // ============================================================
//   yaw = kf.x;
//   if (yaw < 0) yaw += 360.0f;
//   if (yaw >= 360.0f) yaw -= 360.0f;

//   // FIX #4: Depth dengan EMA filter
//   //   SEBELUM: depth = sensor.depth(); lalu if (delta_depth > 1) depth = last_depth;
//   //   SESUDAH: EMA smooth, reject hanya extreme (> 10m)
//   sensor.read();
...
//   // Calculate errors — TANPA dead zone
...
//   //     → error 2cm langsung jadi 0, thruster berhenti koreksi!
//   //   SESUDAH: Tidak ada. PID handle semua error secara natural.

//   // FIX #5: Reset PID saat ganti status/mode
//   //   Mencegah integral windup dari mode sebelumnya
...
//   // FIX #2: Gunakan PID GLOBAL
//   //   SEBELUM: PID pid_yaw(kp_yaw, ki_yaw, kd_yaw); ← BARU setiap loop!
//   //            integral = 0 setiap loop → I-term tidak pernah bekerja
//   //            last_error = 0 setiap loop → D-term spike setiap loop
//   //   SESUDAH: pid_yaw_ctrl sudah ada sejak program start, integral accumulate
...
//   // Debug output
//   Serial.println("----------------------");
//   Serial.print("KF yaw    = "); Serial.println(yaw);
//   Serial.print("Comp raw  = "); Serial.println(compassYaw);
...
//   // FIX #6: HAPUS delay(5) — timer sudah atur timing 20Hz
//   // delay(5);  ← DIHAPUS
// }
