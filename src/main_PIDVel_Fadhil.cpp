...
//       : kp(kp), ki(ki), kd(kd), integ(0), last_err(0) {}

//     float update(float target_vel, float measured_vel, float dt) {
//       float err = target_vel - measured_vel;

//       integ += err * dt;
//       integ = constrain(integ, -50, 50); // anti windup
...
// float positionErrorToVelocity(float error_deg) {
//     const float ERR_MAX = 60.0;      // deg
//     const float ERR_DEADBAND = 2.0;  // deg
//     const float VEL_MAX = 25.0;      // deg/s

//     float abs_err = fabs(error_deg);

//     // Deadband
//     if (abs_err < ERR_DEADBAND)
//         return 0.0;

//     // Jika error sangat besar → velocity diperkecil
//     if (abs_err >= ERR_MAX)
//         return copysign(5.0, error_deg);  // creeping rotation

//     // Linear mapping (20° → vel max)
//     float vel = (abs_err / 20.0) * VEL_MAX;

//     // Clamp
//     vel = constrain(vel, 5.0, VEL_MAX);

//     return copysign(vel, error_deg);
// }
